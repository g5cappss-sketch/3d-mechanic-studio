...
function packCluster(clickedPartId) {
  ...
  packPartsByIds(currentClusterPartIds, clickedPartId);
}

function packPartsByIds(partIds, anchorPartId = partIds[0]) {
  unpackCluster();

  currentClusterPartIds = Array.from(new Set(partIds));
  const clickedPart = parts.find(p => p.id === anchorPartId);
