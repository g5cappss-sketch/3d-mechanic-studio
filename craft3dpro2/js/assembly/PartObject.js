...
class PartObject {
	...
	addSocket(socketData) {
		...
		marker.userData = {
			...
			insertionDepth: socket.insertionDepth,
			locking: socket.locking,
			partId: this.id
		};
...
function normalizeSocket(data, index = 0) {
	...
	return {
		...
		displayIndex: getSocketDisplayIndex(data, index),
		type: data.type === 'male' ? 'male' : 'female',
		radius: Number(data.radius) || (14 / 47) * 0.98,
		position,
		axis,
		insertionDepth: Number(data.insertionDepth) || 0,
		locking: data.locking !== false,
		legacyIndex: data.legacyIndex
	};
}

function getSocketDisplayIndex(data, index) {
	if (Number.isFinite(Number(data.number))) return Number(data.number);
	if (Number.isFinite(Number(data.displayIndex))) return Number(data.displayIndex);
	if (Number.isFinite(Number(data.index))) return Number(data.index) + 1;

	const sourceName = String(data.id || data.name || '');
	const suffix = sourceName.match(/(?:socket|hole|pin)[_-]?(\d+)$/i);
	return suffix ? Number(suffix[1]) : index + 1;
}
