function initScene() {
  ...
  if (typeof initPhysicsManager === 'function') initPhysicsManager();
...
// HÀM RESET MÀU PHÁT SÁNG CỦA TẤT CẢ VẬT THỂ
function resetAllEmissive() {
  parts.forEach(p => {
    if (p.root) {
      p.root.traverse(node => {
...
function animate() {
  ...
  if (selectedPartId) {
    ...
    if (p && p.root) {
      ...
      // FIX LỖI MỜ NHẠT: Tăng cường độ phát sáng lên gấp đôi
        ...
        if (node.isMesh && node.material && !node.userData.isHoleAnchor) {
          if (node.material.emissive) {
            if (typeof isColliding !== 'undefined' && isColliding) {
               node.material.emissive.setHex(0xff0000); // Đỏ rực
            } else {
               // Chuyển sang sáng rõ ràng hơn (Lightness cao hơn)
               node.material.emissive.setHSL(0.55, 1.0, 0.25 + pulse * 0.35); 
            }
          }
        }
      });
      
      if (typeof holeBadgesGroup !== 'undefined' && holeBadgesGroup.children.length > 0) {
