...
function update3DHoleBadges(part) {
  ...
  addSocketBadgesForPart(part);
}

// TÌM VÀ THAY THẾ HÀM NÀY TRONG FILE: 5_interaction.js
function addSocketBadgesForPart(part) {
  if (!part) return;

  const sockets = part.sockets && part.sockets.length > 0
    ? part.sockets
    : (part.holes || []).map(hole => ({
      id: `hole_${hole.index + 1}`,
      position: new THREE.Vector3(hole.x, hole.y || part.height / 2, hole.z || 0),
      legacyIndex: hole.index
    }));
  if (sockets.length === 0) return;

  sockets.forEach((socket, index) => {
    const position = socket.position || new THREE.Vector3(0, part.height / 2, 0);
    const axis = socket.axis || new THREE.Vector3(0, 1, 0);
    const badgePosition = position.clone().add(axis.clone().normalize().multiplyScalar(0.18));
    
    // KIỂM TRA LỖ NÀY ĐÃ BỊ CHIẾM (CÓ CHỐT) CHƯA?
    const occupied = typeof connectionManager !== 'undefined' &&
      connectionManager.isSocketOccupied(part.id, socket.id);

    // Tạo màu nền: Nếu bận thì Xám (#475569), nếu rỗng thì Xanh (#0284c7)
    const bgColor = occupied ? '#475569' : '#0284c7';
    const sprite = createTextSprite(`${socket.displayIndex || index + 1}`, bgColor, '#ffffff');
    
    if (occupied) {
      sprite.material.opacity = 0.35; // Làm mờ đi
    }
    ...
    sprite.userData = {
      ...
      socketId: socket.id,
      occupied: occupied,
      clickable: !occupied, // RẤT QUAN TRỌNG: Khóa click nếu đã có chốt
      localX: badgePosition.x,
      localY: badgePosition.y,
      localZ: badgePosition.z
    };
...
function updateAllSocketBadges() {
  while (holeBadgesGroup.children.length > 0) {
    holeBadgesGroup.remove(holeBadgesGroup.children[0]);
  }
  parts.forEach(part => {
    if (part.root && part.root.visible) addSocketBadgesForPart(part);
  });
}
