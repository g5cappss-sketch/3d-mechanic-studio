    ...
    if (p && p.root) {
      ...
      holeBadgesGroup.children.forEach(sprite => {
        if (sprite.userData && sprite.userData.localX !== undefined) {
          const badgePart = parts.find(part => part.id === sprite.userData.partId) || p;
          if (!badgePart || !badgePart.root) return;
          badgePart.root.updateMatrixWorld(true);
          ...
          localPos.applyMatrix4(badgePart.root.matrixWorld);
