...
function initSelectionBox() {
  if (!canvas || canvas.dataset.selectionReady === 'true') return;
  canvas.dataset.selectionReady = 'true';
  canvas.addEventListener('pointerdown', startSelectionBoxOnEmptySpace, true);
  canvas.addEventListener('pointermove', updateSelectionBox);
  canvas.addEventListener('pointerup', finishSelectionBox);
  canvas.addEventListener('pointercancel', finishSelectionBox);
}

function startSelectionBoxOnEmptySpace(event) {
  if (event.button !== 0 || currentToolMode !== 'select' || selectionBoxState) return;
  if (typeof transformControl !== 'undefined' && transformControl && transformControl.dragging) return;

  if (typeof getHingeHandleHit === 'function' && typeof beginHingeHandleDrag === 'function') {
    const hingeHandle = getHingeHandleHit(event);
    if (hingeHandle && beginHingeHandleDrag(event, hingeHandle.connectionId)) {
  ...
  // Badge socket nam ngoai than model phai duoc uu tien cho thao tac ghep truc tiep.
  const badgeHits = typeof holeBadgesGroup !== 'undefined'
    ? raycaster.intersectObjects([holeBadgesGroup], true)
    : [];
  if (badgeHits.some(hit => hit.object && hit.object.userData && hit.object.userData.isSocketBadge)) {
    if (typeof handleFreeAssemblyPointerDown === 'function' && handleFreeAssemblyPointerDown(event)) {
      event.preventDefault();
      event.stopImmediatePropagation();
    }
    return;
  ...
  // Shift/Ctrl cho phép khoanh vùng ngay cả khi bắt đầu trên một linh kiện.
  // Không có phím bổ trợ thì click trên linh kiện vẫn giữ hành vi chọn đơn.
  if (intersects.length > 0 && !event.shiftKey && !event.ctrlKey) return;
...
function beginSelectionBox(event) {
  if (event.button !== 0 || currentToolMode !== 'select') return;
  const rect = canvas.getBoundingClientRect();
  selectionBoxState = {
...
function updateSelectionBox(event) {
  if (!selectionBoxState || event.pointerId !== selectionBoxState.pointerId) return;
  const rect = canvas.getBoundingClientRect();
  ...
  selectionBoxState.moved = width > 6 || height > 6;
  ...
  box.style.display = selectionBoxState.moved ? 'block' : 'none';
}
...
function finishSelectionBox(event) {
  ...
  const right = Math.max(state.startX, endX);
  const top = Math.min(state.startY, endY);
  const bottom = Math.max(state.startY, endY);
  const selectedIds = parts.filter(part => {
    if (!part.root || !part.root.visible) return false;
    const projected = part.root.getWorldPosition(new THREE.Vector3()).project(camera);
  ...
  if (selectedIds.length > 0) {
    if (event.shiftKey || event.ctrlKey) {
      const existingIds = typeof currentClusterPartIds !== 'undefined' ? currentClusterPartIds : [];
      selectParts(Array.from(new Set(existingIds.concat(selectedIds))));
    } else {
...
function selectParts(ids) {
  const validIds = ids.filter(id => parts.some(part => part.id === id));
  if (validIds.length === 0) return;
  const primary = parts.find(part => part.id === validIds[0]);
  selectedPartId = primary.id;
  if (typeof packPartsByIds === 'function') packPartsByIds(validIds, primary.id);
  if (typeof applyGlowEffect === 'function') applyGlowEffect(validIds);
  const title = document.getElementById('inspect-part-title');
  const idLabel = document.getElementById('inspect-part-id');
  if (title) title.innerText = validIds.length > 1 ? `${validIds.length} chi tiết được chọn` : primary.name;
  if (idLabel) idLabel.innerText = validIds.length > 1 ? 'Vùng chọn độc lập' : `Loại: ${primary.type} • ID: ${primary.id}`;
  const noSelection = document.getElementById('inspector-no-selection');
...
window.selectPart = function(id) {
  if (typeof selectJoinObject === 'function' && selectJoinObject(id)) return;
  selectedPartId = id;
  ...
  // Cho phép chốt đã gắn được tháo ra và kéo sang lỗ khác.
  if (part.type === 'pin' && typeof connectionManager !== 'undefined') {
    connectionManager.detachPart(part.id);
  }
  ...
  if (socketsContainer) {
    socketsContainer.innerHTML = '<span class="text-[10px] text-slate-500">Chọn hai đối tượng ở tab Ghép 2, sau đó chọn socket bằng danh sách bên trên.</span>';
  }
